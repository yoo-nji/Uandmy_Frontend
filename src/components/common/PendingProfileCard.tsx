...
const ProfileCard = ({ ProfileDatas, handleAccept }: ProfileCardProps) => {
  return (
    <>
      {ProfileDatas.map((profile, index) => (
        <div
          key={index}
          className="w-full h-auto p-3 border-2 border-[#E9E9E9] rounded-lg space-y-2">
          <div className="flex flex-row gap-2">
            <Avartar size={56} />

            <div>
              <p className="font-semibold text-base">{profile.name}</p>
              <p className="text-[0.813rem] text-[#65657E]">{profile.job}</p>
